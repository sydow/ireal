...
-- Magic numbers, used in solve below ------------------------------------------
...
odeDerivs :: (Num a, Num b, VarPrec b) => (Dif a -> Dif b -> Dif b) -> a -> b -> [b]
odeDerivs f t0 x0 = fromDif x
    where xder = f (var t0) x
          x = mkDif x0 xder

{-
-- We multiply with facs here and then use generalterms which has divided by facs...
-- odeDerivs :: (Num a, Num b) => (Dif a -> Dif b -> Dif b) -> a -> b -> [b]
odeDerivs f t0 x0 = zipWith (*) xs facs --fromDif x
    where D xder = f (var t0) (D xs)
          xs =  x0 : zipWith (/) xder [1..]
-}
...
solve f p n ((h,s):ps) = rs ++ solve f (head us) n ps
   where (rs,us) = splitAt s (iterate (g h) p)
         g h (t,x) = (t+h, prec dec (hull [step (t, lower x), step (t, upper x)]))
         hs = generalTerms h
         step (t,x) = sum (take n $ zipWith (*) (odeDerivs f t x) hs) + err
          where ti = t -+- (t+h)
                xi = bound initRad Nothing 
                err = odeDerivs f ti xi !! n * hs !! n

                bound rad maybeAns
                 | rad > maxRad = error "Cannot verify existence"
                 | i1 `containedIn` i0 `atDecimals` (dec+2) = bound (rad/2) (Just i0)
...
psolve f (t0,x0) n hns = plotData [fu, fl] t0 xs
    where ss = solve f (t0,x0) n hns 
          xs = t0 + sum (map (\(h,n) -> h * fromIntegral n) hns)
          fu = map (\(t,x) -> showIReal 10 t ++ " " ++ showIReal 10 (upper x)) ss
          fl = map (\(t,x) -> showIReal 10 t ++ " " ++ showIReal 10 (lower x)) ss
